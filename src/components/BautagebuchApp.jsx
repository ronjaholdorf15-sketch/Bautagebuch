...
export default function BautagebuchApp({ username, ncUrl, ncUser, ncPassword }) {
  ...
  // Ordnerliste aus Nextcloud laden
  const loadFolders = async () => {
    ...
    setLoading(true);
    try {
      // URL unverändert lassen, nur Benutzername & Passwort kodieren
    ...
    <div className="max-w-2xl mx-auto bg-white shadow-lg p-6 rounded-2xl mt-8">
      ...
      <div className="grid gap-3 mb-4">
        ...
        <div className="flex flex-col gap-2">
          ...
          />
          {images.length > 0 && (
            <p className="text-sm">{images.length} Bild(er) ausgewählt</p>
          )}
        </div>
      </div>
